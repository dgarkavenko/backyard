namespace Systems
{
	...
	{
		/**
		 * Reads: DetectionHot, AimHot, AimCold
		...
		void Tick(FBSRuntimeStore& Store, float DeltaSeconds)
		{
			for (int DetectionIndex = 0; DetectionIndex < Store.DetectionHot.Num(); DetectionIndex++)
			{
				...
				AimHot.bHasAimTarget = DetectionHot.VisionState == EBSSentryVisionState::Tracking || DetectionHot.VisionState == EBSSentryVisionState::LostHold;
				AimHot.bUseProbe = DetectionHot.VisionState == EBSSentryVisionState::Probing || DetectionHot.VisionState == EBSSentryVisionState::Acquiring;
				AimHot.bHasConfirmation = DetectionHot.VisionState == EBSSentryVisionState::Tracking;

				AimHot.AimTargetLocation = DetectionHot.CurrentTargetLocation;
				AimHot.ProbeYawTarget = DetectionHot.ProbeTargetYaw;
			}
		...
		void ReadMuzzle(const FBSAimColdRow& AimCold, FBSAimHotRow& AimHot)
		{
			...
			AimHot.MuzzleError = DistanceToTarget > 0.0f ? (TargetDirection.Rotation() - MuzzleWorldRotation).GetNormalized() : FRotator(0, 0, 0);
		}
